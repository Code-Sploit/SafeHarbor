...
static unsigned int in_aton(const char *ip_str);

static char *get_current_time_string(void);
static char *ip_ntoa(unsigned int ip);

static int modify_rule(int index, const char *new_rule_str);
static int add_rule(const char *rule_str);
static int parse_rule(char *rule);
static int remove_rule(int index);
...
static long int bridge_ctl(struct file *file, unsigned int cmd, unsigned long arg)
{
    ...
    copy_from_user(arg_str, (char *) arg, MAX_STR_LEN);

    // Handle different ioctl commands
    switch (cmd)
    {
        case BRIDGE_ADD_RULE:
            break;

        case BRIDGE_DEL_RULE:
            break;
...
unsigned int in_aton(const char *ip_str)
{
    unsigned int ip = 0;
...
static int __init firewall_init(void)
{
    ...
    printk(KERN_INFO "SafeHarbor: Initializing IOCTL bridge\n");
...
static unsigned int filter(void *priv, struct sk_buff *skb, const struct nf_hook_state *state)
{
    ...
    for (int i = 0; i < num_rules; i++)
    {
        ...
        {
            if ((strcmp(rule_protocol, "tcp") == 0 || strcmp(rule_protocol, "udp") == 0) &&
                (strcmp(rule_direction, (direction == 0) ? "out" : "in") == 0 || strcmp(rule_direction, "any") == 0) &&
                (strcmp(rule_src_ip, "any") == 0 || strcmp(rule_src_ip, ip_ntoa(src_ip)) == 0) &&
                (strcmp(rule_src_port, "any") == 0 || strcmp(rule_src_port, src_port_str) == 0) &&
                (strcmp(rule_dest_ip, "any") == 0 || strcmp(rule_dest_ip, ip_ntoa(dest_ip)) == 0) &&
                (strcmp(rule_dest_port, "any") == 0 || strcmp(rule_dest_port, dest_port_str) == 0))
            {              
                drop = 0;
                rule_active = i + 1;
            }
    ...
    if (drop == 1)
    {
        log_message("PACKET: [%s] [%s] [%s] [%s] [%s] [%s] [%s] by rule [%d]\n",
                    (protocol == IPPROTO_TCP) ? "tcp" : "udp",
                    (direction == 0) ? "out" : "in",
                    ip_ntoa(src_ip),
                    src_port_str,
                    ip_ntoa(dest_ip),
                    dest_port_str, "DROPPED", rule_active);
    }
    else if (drop == 0)
    {
        if (rule_active != 0)
        {
            log_message("PACKET: [%s] [%s] [%s] [%s] [%s] [%s] [%s] by rule [%d]\n",
                ...
                src_port_str,
                ip_ntoa(dest_ip),
                dest_port_str, "ACCEPTED", rule_active);
        }
        else
        {
            if (DO_SHOW_RULE_MISMATCHES == 1)
            {
                log_message("PACKET: [%s] [%s] [%s] [%s] [%s] [%s] [%s] by rule mismatch\n",
                    (protocol == IPPROTO_TCP) ? "tcp" : "udp",
                    (direction == 0) ? "out" : "in",
                    ip_ntoa(src_ip),
                    src_port_str,
                    ip_ntoa(dest_ip),
                    dest_port_str, "ACCEPTED");
            }
        }
    }
...
int remove_rule(int index)
{
    ...
    {
        memcpy(&rules[i], &rules[i + 1], sizeof(struct Rule));
    }
...
int modify_rule(int index, const char *new_rule_str)
{
    if (index < 0 || index >= num_rules)
    {
        return -EINVAL;
    ...
    strcpy(rules[index].action, "");
    strcpy(rules[index].protocol, "");
    strcpy(rules[index].direction, "");
    strcpy(rules[index].src_ip, "");
    strcpy(rules[index].src_port, "");
    strcpy(rules[index].dest_ip, "");
    strcpy(rules[index].dest_port, "");

    parse_rule(new_rule_str);
